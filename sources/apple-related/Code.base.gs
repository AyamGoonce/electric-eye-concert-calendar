...
function eeAppleSearch_(query) {
  var params = {
    term: query.term,
    country: String(query.storefront || EE_APPLE_CONFIG.storefront).toLowerCase(),
    media: query.media,
    entity: query.entity,
    limit: Math.min(EE_APPLE_CONFIG.searchLimit, 200),
    lang: "fr_fr",
    explicit: "Yes"
  };
  ...
  var cached = cache.get("apple-search:" + digest);
  ...
  try {
    ...
    var wait = Math.max(0, EE_APPLE_CONFIG.minimumRequestIntervalMs - (Date.now() - last));
    ...
    if (response.getResponseCode() !== 200) throw new Error("Apple Search API returned " + response.getResponseCode());
    var value = JSON.parse(response.getContentText());
    var cacheText = JSON.stringify(value);
    if (cacheText.length < 95000) cache.put("apple-search:" + digest, cacheText, 21600);
    return value;
...
function eeAppleLookup_(query) {
  ...
  var cache=CacheService.getScriptCache();
  var cacheKey="apple-lookup:"+digest;
  ...
  try{
    ...
    var wait=Math.max(
      0,
      EE_APPLE_CONFIG.minimumRequestIntervalMs-(Date.now()-last)
    );
    ...
    if(response.getResponseCode()!==200){
      throw new Error("Apple Lookup API returned "+response.getResponseCode());
    }
    ...
    if(cacheText.length<95000){
      cache.put(cacheKey,cacheText,21600);
    }
...
function eeAppleTvPush_(items, seen, raw, storefront, subject) {
  ...
  if(!id||!title||!url||seen[id])return;
  if(!subject||!eeContains_(title,subject))return;
  ...
  items.push({
    ...
    creator:"",
    mediaType:String(raw.mediaType||"Film"),
    description:"",
    storefront:String(storefront||"FR").toUpperCase(),
    category:"WATCH",
    relevanceTier:"DIRECT",
    relevanceScore:96,
    relevanceReason:"Apple TV title directly concerns the article subject.",
    relationshipContext:"Apple TV title directly concerns the article subject.",
    price:null,
    discoverySource:"APPLE_TV_PUBLIC_SEARCH"
  });
}
...
function eeBackfillBatch() {
  ...
  posts.forEach(function(post) {
    try {
      ...
      if (existing && Array.isArray(existing.categories) && existing.categories.length) {
        results.push({
          postId:String(post.id),
          title:post.title,
...
function eeProcessNewestPosts() {
  ...
  posts.forEach(function(post) {
    try {
      ...
      if (existingNewest && Array.isArray(existingNewest.categories) && existingNewest.categories.length) {
...
function eeAppleLabelFallbackArtists_(post) {
  var generic=/^(?:news|review|album review|record review|concert|concert review|live report|live review|interview|obituary|r\.?i\.?p\.?|music|rock|hard rock|classic rock|alternative rock|indie rock|progressive rock|prog|blues|blues rock|metal|heavy metal|country|folk|americana|pop|punk|jazz|electronic|festival|tour|video|photos?|playlist|friday'?s playlist)$/i;
...
function eeResolveIdentity_(analysis, musicResults) {
  var subject=analysis.primaryArtists[0]||"", existingId=String((analysis.existingAppleArtistIds||[])[0]||""), mappings=eeIdentityMappings_(), scores={};
...
function eeSearchPlan_(analysis, storefront) {
  ...
  var primary=analysis.primaryArtists[0]||"",plan=[],seen={};
    ...
    try{
      ...
      expandedAlbums=(expandedResponse.results||[]).filter(function(raw){
        return raw.collectionId &&
          eeNorm_(raw.artistName||"")===(analysis.primaryArtists[0]||"");
      });
  ...
  if(identity.level==="HIGH"){
    Object.keys(map).forEach(function(key){
      ...
      if(
        ...
        eeNorm_(item.creator||"")===(analysis.primaryArtists[0]||"")
      ){
        delete map[key];
      }
    });
...
function eeDebugBackfill9() {
  var posts = eeFetchPosts_(9, 1);
  if (!posts.length) {
    console.log("NO POST");
    return;
  }
...
function eeDebugSignals9() {
  ...
  console.log(JSON.stringify({
    title: post.title,
    labels: post.labels || [],
    signals: eeEntitySignalCandidates_(post),
    fallbackArtists: eeAppleLabelFallbackArtists_(post)
  }, null, 2));
}
function eeDebugAnalysis9() {
  var post = eeFetchPosts_(9, 1)[0];
  Logger.log(JSON.stringify(eeArticleAnalysis_(post), null, 2));
}


function eeFinalTest9() {
  var post = eeFetchPosts_(9, 1)[0];
  var payload = eeProcessPost_(post);
  ...
  console.log(JSON.stringify({
    title: post.title,
    primaryArtists: payload.subject && payload.subject.primaryArtists,
